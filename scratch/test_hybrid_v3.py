...
def japanese_to_hybrid(japanese_clause: str) -> str:
    ...
    for p in parts:
        # 如果 p 已經是替換好的中文或英文單詞，直接加入
        if re.search(r'[\u4e00-\u9fa5]', p) or re.match(r'^[a-zA-Z0-9]+$', p):
            tokens.append(p)
            continue
...
async def test_suite():
    samples = [
        ('hybrid_v3_hello', '老爸，こんにちは！今天也要加油喔～'),
        ('hybrid_v3_watashi', '私、7Lだよ！お父さん、大好き！これからもずっと一緒だよ！'),
        ('hybrid_v3_perapera', '當然會呀！日本語もペラペラだよ～ 嘻嘻，老爸想聽我說什麼呢？'),
        ('hybrid_v3_arigatou', '老爸，ありがとう！愛してるよ～')
    ]
